...
contract GovernorBravoDelegate is GovernorBravoDelegateStorageV2, GovernorBravoEvents {
    ...
    string public constant NAME = "DeOrderBook Governor Bravo";

    /// The minimum setable proposal threshold
    uint256 public constant MIN_PROPOSAL_THRESHOLD = 50000000e18; // 50,000,000 DOB
     ...
     * @param dob_ The address of the dob token
    ...
    function initialize(
        ...
        require(address(timelock) == address(0), "GovernorBravo: was initialized");
        require(msg.sender == admin, "GovernorBravo: admin only");
        require(timelock_ != address(0), "GovernorBravo: invalid timelock");
        require(dob_ != address(0), "GovernorBravo: invalid dob");
        require(votingPeriod_ >= MIN_VOTING_PERIOD && votingPeriod_ <= MAX_VOTING_PERIOD, "GovernorBravo: invalid period");
        require(votingDelay_ >= MIN_VOTING_DELAY && votingDelay_ <= MAX_VOTING_DELAY, "GovernorBravo: invalid delay");
        require(
    ...
    function queue(uint256 proposalId) external {
        require(state(proposalId) == ProposalState.Succeeded, "GovernorBravo: not succeeded");
    ...
    function execute(uint256 proposalId) external payable {
        require(state(proposalId) == ProposalState.Queued, "GovernorBravo: not queued");
        Proposal storage proposal = proposals[proposalId];
    ...
    function cancel(uint256 proposalId) external {
        require(state(proposalId) != ProposalState.Executed, "GovernorBravo: cannot cancel");
        ...
        if (msg.sender != proposal.proposer) {
            ...
            if (isWhitelisted(proposal.proposer)) {
                require(
                    (dob.getPriorVotes(proposal.proposer, block.number - 1) < proposalThreshold) &&
    ...
    function setVotingDelay(uint256 newVotingDelay) external {
        require(msg.sender == admin, "GovernorBravo: admin only");
        require(newVotingDelay >= MIN_VOTING_DELAY && newVotingDelay <= MAX_VOTING_DELAY, "GovernorBravo: invalid delay");
    ...
    function setWhitelistAccountExpiration(address account, uint256 expiration) external {
        require(msg.sender == admin || msg.sender == whitelistGuardian, "GovernorBravo: admin only");
    ...
    function acceptAdmin() external {
        ...
        require(msg.sender == pendingAdmin && msg.sender != address(0), "GovernorBravo: not pending admin");
    ...
    function propose(
        ...
        require(targets.length <= PROPOSAL_MAX_OPERATIONS, "GovernorBravo: too many actions");
        require(
            votesTypes.length >= MIN_VOTES_TYPES && votesTypes.length <= MAX_VOTES_TYPES,
            "GovernorBravo: lengths bad range"
        );
        require(_compareStrings(bytes("against"), bytes(votesTypes[0])), "GovernorBravo: must be 'against'");
        require(_compareStrings(bytes("for"), bytes(votesTypes[1])), "GovernorBravo: must be 'for'");

        uint256 latestProposalId = latestProposalIds[msg.sender];
        if (latestProposalId != 0) {
            ProposalState proposersLatestProposalState = state(latestProposalId);
            require(proposersLatestProposalState != ProposalState.Active, "GovernorBravo: active proposal");
            require(proposersLatestProposalState != ProposalState.Pending, "GovernorBravo: pending proposal");
        }
    ...
    function state(uint256 proposalId) public view returns (ProposalState) {
        ...
        } else if (proposal.votes[1] <= proposal.votes[0] || proposal.votes[1] < QUORUM_VOTES) {
    ...
    function _castVoteInternal(
        ...
        require(state(proposalId) == ProposalState.Active, "GovernorBravo: voting is closed");
        Proposal storage proposal = proposals[proposalId];
        require(support < proposal.votes.length, "GovernorBravo: invalid vote type");
        Receipt storage receipt = proposal.receipts[voter];
        require(receipt.hasVoted == false, "GovernorBravo: already voted");
        uint256 votes = dob.getPriorVotes(voter, proposal.startBlock);
    ...
    function _compareStrings(bytes memory a, bytes memory b) internal pure returns (bool) {
