...
contract Option is Ownable, ReentrancyGuard, IOption {
     ...
     * @dev Set the ratio used to calculate the fee for entering the option. The value must be between 0 and 9999.
     * @param _feeRatio The new entry fee ratio.
     */
    function setOptionEntryFeeRatio(uint16 _feeRatio) external override onlyFactory {
        require(0 <= _feeRatio && _feeRatio < 10000, "Option: Illegal value range");
        optionEntryFeeRatio = _feeRatio;
    }

    /**
     * @notice Set the exercise fee ratio for the option.
     * @dev Set the ratio used to calculate the fee for exercising the option. The value must be between 0 and 9999.
     ...
     * @dev Set the ratio used to calculate the fee for withdrawing tokens from the option. The value must be between 0 and 9999.
     ...
     * @dev Set the ratio used to calculate the fee for redeeming tokens from the option. The value must be between 0 and 9999.
    ...
    function setAllRatio(
        ...
        if (_entryFeeRatio >= 0 && _entryFeeRatio < 10000) {
            optionEntryFeeRatio = _entryFeeRatio;
        }
        if (_exerciseFeeRatio >= 0 && _exerciseFeeRatio < 10000) {
            optionExerciseFeeRatio = _exerciseFeeRatio;
        }
        if (_withdrawFeeRatio >= 0 && _withdrawFeeRatio < 10000) {
            optionWithdrawFeeRatio = _withdrawFeeRatio;
        }
        if (_redeemFeeRatio >= 0 && _redeemFeeRatio < 10000) {
